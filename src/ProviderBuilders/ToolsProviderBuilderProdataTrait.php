...
trait ToolsProviderBuilderProdataTrait
{
    /**
     * @param array|string $payload
     */
    private function buildProdataEnvelope(array|string $payload, string $service): string
    {
        $data = $this->normalizePayload($payload);
        $method = $this->mapProdataMethod($service);
        ...
        return '<?xml version="1.0" encoding="UTF-8"?>'
            . '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:ser="http://services.nfse">'
            . '<soapenv:Header/>'
            . '<soapenv:Body>'
            . '<ser:' . $method . 'Request>'
    ...
    private function mapProdataMethod(string $service): string
    {
    ...
    private function buildProdataSoapAction(string $service): string
    {
        return 'http://services.nfse/' . $this->mapProdataMethod($service);
    }
}
