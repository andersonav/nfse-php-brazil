...
trait ToolsProviderBuilderSaatriTrait
{
    /**
     * @param array|string $payload
     */
    private function buildSaatriEnvelope(array|string $payload, string $service, ?string $versao): string
    {
        $data = $this->normalizePayload($payload);
        [$method, $methodRequest] = $this->mapSaatriMethod($service);
        $dados = trim((string) ($data['dados_xml'] ?? ''));
        if ($dados === '') {
            $dados = $this->buildAbrasfDataForMethod($data, $method);
        }
        $cabecalho = $this->buildAbrasfCabecalhoXml($versao ?: '2.01');
        $wsse = $this->buildWsseUsernameTokenHeader($data);

        $inner = '<nfse:' . $methodRequest . '>'
     ...
     */
    private function mapSaatriMethod(string $service): array
    {
    ...
    private function buildSaatriSoapAction(string $service): string
    {
        [$method] = $this->mapSaatriMethod($service);
        return 'http://nfse.abrasf.org.br/Infse/' . $method;
    }
}
